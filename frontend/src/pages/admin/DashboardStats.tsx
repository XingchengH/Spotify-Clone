...
export default function DashbordStats() {
  const numberOfSongs  = useSelector((state: RootState) => state.songs?.songs?.length  || 0);
  const numberOfAlbums = useSelector((state: RootState) => state.albums?.albums?.length || 0);
